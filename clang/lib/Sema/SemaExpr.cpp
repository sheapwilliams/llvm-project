      ...
      if (!R.empty()) {
        ...
        if (isInstance) {
          Diag(R.getNameLoc(), diagnostic)
              << Name << FixItHint::CreateInsertion(R.getNameLoc(), "this->");
  ...
  if (Lookup.empty() && II && AllowBuiltinCreation) {
    ...
    if (unsigned BuiltinID = II->getBuiltinID()) {
      if (!(getLangOpts().CPlusPlus &&
        ...
        NamedDecl *D = LazilyCreateBuiltin((IdentifierInfo *)II, BuiltinID, S,
...
ExprResult Sema::PerformObjectMemberConversion(Expr *From,
  ...
  if (FoundDecl->getDeclContext() != Member->getDeclContext()) {
    ...
    if (!Context.hasSameUnqualifiedType(FromRecordType, URecordType)) {
      ...
      CXXCastPath BasePath;
      if (CheckDerivedToBaseConversion(FromRecordType, URecordType, FromLoc,
                                       FromRange, &BasePath))
        return ExprError();
      ...
      From = ImpCastExprToType(From, UType, CK_UncheckedDerivedToBase, VK,
                               &BasePath)
                 .get();
      FromType = UType;
  ...
  if (CheckDerivedToBaseConversion(FromRecordType, DestRecordType, FromLoc,
                                   FromRange, &BasePath, IgnoreAccess))
    return ExprError();
...
ExprResult Sema::BuildCompoundLiteralExpr(SourceLocation LParenLoc,
  ...
  Expr *E = new (Context) CompoundLiteralExpr(LParenLoc, TInfo, literalType, VK,
...
static void diagnoseTautologicalComparison(Sema &S, SourceLocation Loc,
  ...
  if (DL && DR && declaresSameEntity(DL, DR)) {
    ...
    S.DiagRuntimeBehavior(Loc, nullptr,
             ...
             DR->getType()->isArrayType() && !DL->isWeak() && !DR->isWeak()) {
...
QualType Sema::CheckCompareOperands(ExprResult &LHS, ExprResult &RHS,
  ...
  if ((LHSType->isIntegerType() && !LHSIsNull) ||
    ...
    if (LCanPointeeTy != RCanPointeeTy) {
      ...
      if (getLangOpts().OpenCL && !LHSIsNull && !RHSIsNull) {
        ...
        if (!LHSPtr->isAddressSpaceOverlapping(
                *RHSType->getAs<PointerType>())) {
          Diag(Loc,
               diag::err_typecheck_op_on_nonoverlapping_address_space_pointers)
  ...
  if (!IsRelational && RHSIsNull && LHSType->isBlockPointerType() &&
      RHSType->isIntegerType()) {
    RHS = ImpCastExprToType(RHS.get(), LHSType, CK_NullToPointer);
    return computeResultTy();
  }
  if (!IsRelational && LHSIsNull && LHSType->isIntegerType() &&
      RHSType->isBlockPointerType()) {
    LHS = ImpCastExprToType(LHS.get(), RHSType, CK_NullToPointer);
    return computeResultTy();
  }
...
inline QualType Sema::CheckBitwiseOperands(ExprResult &LHS, ExprResult &RHS,
  ...
  QualType compType =
      UsualArithmeticConversions(LHSResult, RHSResult, IsCompAssign);
...
static ExprResult convertHalfVecBinOp(Sema &S, ExprResult LHS, ExprResult RHS,
  ...
  if (IsCompAssign)
    return new (Context)
        CompoundAssignOperator(LHS.get(), RHS.get(), Opc, ResultTy, VK, OK,
...
ExprResult Sema::CreateBuiltinBinOp(SourceLocation OpLoc,
  ...
  ConvertHalfVec =
      needsConversionOfHalfVec(ConvertHalfVec, Context, LHS.get()->getType());
...
static ExprResult BuildOverloadedBinOp(Sema &S, Scope *Sc, SourceLocation OpLoc,
  ...
  if (Sc && OverOp != OO_None && OverOp != OO_Equal)
    S.LookupOverloadedOperatorName(OverOp, Sc, LHS->getType(), RHS->getType(),
                                   Functions);
...
bool Sema::DiagnoseAssignmentResult(AssignConvertType ConvTy,
  ...
  Diag(Loc, FDiag);
  if (DiagKind == diag::warn_incompatible_qualified_id && PDecl && IFace &&
      !IFace->hasDefinition())
  ...
  if (getLangOpts().CPlusPlus11) {
    ...
    public:
      CXX11ConvertDiagnoser(bool Silent)
          : ICEConvertDiagnoser(/*AllowScopedEnumerations*/ false, Silent,
                                true) {}
